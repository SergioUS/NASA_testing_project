...
pm.test("Each image URL in the response is valid", function () {
    ...
    docs.forEach((doc) => {
        if (doc.endPoint) {
            pm.expect(doc.endPoint).to.match(/^https:\/\/trek\.nasa\.gov\/moon\/trekarcgis3\/rest\/services\/Unified_Global_Geologic_Map_of_the_Moon_Geologic_Units_NP\/MapServer$/);
        }
    });
});
